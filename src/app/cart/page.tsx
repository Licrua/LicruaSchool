...
export default function CartPage() {
  ...
  useEffect(() => {
    dispatch(setCartStatus('formCart'));
  }, [dispatch]);
      ...
      <div className="max-w-5xl mx-auto p-6">
        ...
        <div className="flex flex-col gap-6">
          {Array.from({ length: cart.length || 3 }).map((_, idx) => (
            <div
              key={idx}
              className="bg-base-100 shadow-lg relative p-5 flex items-center gap-4"
            >
  ...
  return (
    <div className="max-w-5xl mx-auto p-6">
      <Arrow />
      <h1 className="text-4xl font-bold text-end sm:text-center mb-8">Cart of courses</h1>
      {cart.length === 0 ? (
        ...
        <div className="flex flex-col gap-6">
          ...
          {cart.map((item, index) => (
            <div
              key={index}
              className="bg-base-100 relative shadow-lg p-5 flex flex-row items-center justify-between"
            >
              <figure>
                <Image
                  width={300}
